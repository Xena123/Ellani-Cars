...
if ( ! defined( 'WPBC_EMAIL_MODIFICATION_PREFIX' ) )   define( 'WPBC_EMAIL_MODIFICATION_PREFIX',  'booking_email_' ); // Its defined in api-emails.php file & its same for all emails, here its used only for easy coding...

if ( ! defined( 'WPBC_EMAIL_MODIFICATION_ID' ) )       define( 'WPBC_EMAIL_MODIFICATION_ID',      'modification' );      /* Define Name of Email Template.   
                                                                                                                   Note. Prefix "booking_email_" defined in api-emails.php file. 
                                                                                                                   Full name of option is - "booking_email_modification"
...
function wpbc_import6_is_old_email_modification_exist() {
    
    $is_old_data_exist = get_bk_option( 'booking_email_modification_content' );
...
function wpbc_import6_get_old_email_modification_data() {
    
    $default_values = array();
    if ( wpbc_import6_is_old_email_modification_exist() ) {
                
        $default_values['enabled']  = get_bk_option( 'booking_is_email_modification_adress' );
        $default_values['copy_to_admin']       = get_bk_option( 'booking_is_email_modification_send_copy_to_admin' );          //Parse it to  Name and Email 
        $default_values['from']     = get_bk_option( 'booking_email_modification_adress' );     //Parse it to  Name and Email, relpace [visitoremail]
        $default_values['subject']  = get_bk_option( 'booking_email_modification_subject' );
        $default_values['content']  = get_bk_option( 'booking_email_modification_content' );
...
function wpbc_import6_is_new_email_modification_exist() {
    
    $is_data_exist = get_bk_option( WPBC_EMAIL_MODIFICATION_PREFIX . WPBC_EMAIL_MODIFICATION_ID );           // ''booking_email_' - defined in api-emails.php  file.
...
function wpbc_import6_email__modification__get_fields_array_for_activation() {
    
           
    if ( ! wpbc_import6_is_new_email_modification_exist() ) {                      // New Email Template NOT exist
    
        // Import
        $old_data = wpbc_import6_get_old_email_modification_data();
...
//            $old_data['content'] = html_entity_decode( $old_data['content'] );
        ...
        $mail_api = new WPBC_Emails_API_Modification( WPBC_EMAIL_MODIFICATION_ID , $init_fields_values );    
        
        $default_fields_in_settings = $mail_api->get_default_values() ;         // Get default fields values, from settings - defined in function init_settings_fields {class WPBC_Emails_API_Modification}

        $fields_values = wp_parse_args( $old_data, $default_fields_in_settings );
        
        return array( WPBC_EMAIL_MODIFICATION_PREFIX . WPBC_EMAIL_MODIFICATION_ID => $fields_values );
        
        /*
        Array ( 
                [booking_email_modification] => Array
                                                (
                                                    [enabled] => On
        ...
        return array( WPBC_EMAIL_MODIFICATION_PREFIX . WPBC_EMAIL_MODIFICATION_ID => array() );
    }
}
// </editor-fold>
...
/** Email   F i e l d s  */
class WPBC_Emails_API_Modification extends WPBC_Emails_API  {                       // O v e r r i d i n g     "WPBC_Emails_API"     ClASS
    ...
    public function init_settings_fields() {
        ...
        $this->fields['subject'] = array(   
                                      'type'        => 'text'
                                    , 'default'     => __( 'The reservation has been modified', 'booking' )
                                    //, 'placeholder' => ''
        ...
        $email_content = sprintf( __( 'The reservation %s for: %s has been modified. %sYou can edit this booking on this page: %s  Thank you, %s', 'booking' ), '[bookingtype]', '[dates]', '<br/><br/>[content]<br/><br/>', '[visitorbookingediturl]<br/><br/>', $blg_title . '<br/>[siteurl]' );
...
class WPBC_Settings_Page_Email_Modification extends WPBC_Page_Structure {
    ...
    public function mail_api( $selected_email_name ='',  $init_fields_values = array() ){
        ...
        if ( $this->email_settings_api === false ) {
            $this->email_settings_api = new WPBC_Emails_API_Modification( $selected_email_name , $init_fields_values );    
        }
...
//                    );
        ...
        $is_data_exist = get_bk_option( WPBC_EMAIL_NEW_ADMIN_PREFIX . WPBC_EMAIL_MODIFICATION_ID );
        ...
        $subtabs['modification'] = array( 
                            'type' => 'subtab'                                  // Required| Possible values:  'subtab' | 'separator' | 'button' | 'goto-link' | 'html'
                            , 'title' =>  $icon . __('Modified' ,'booking') . $sufix        // Title of TAB    
                            , 'page_title' => __('Emails Settings', 'booking')  // Title of Page   
                            , 'hint' => __('Customization of email template, which is sending after modification of booking' ,'booking')   // Hint    
                            , 'link' => ''                                      // link
                            , 'position' => ''                                  // 'left'  ||  'right'  ||  ''
                            , 'css_classes' => ''                               // CSS class(es)
...
//debuge ( wpbc_import6_email__modification__get_fields_array_for_activation() );
        ...
        $init_fields_values = wpbc_import6_email__modification__get_fields_array_for_activation();
        ...
        $this->mail_api( WPBC_EMAIL_MODIFICATION_ID, $init_fields_values );
            ...
            <form  name="<?php echo $submit_form_name; ?>" id="<?php echo $submit_form_name; ?>" action="" method="post" autocomplete="off">
                ...
                <div class="metabox-holder">
                    ...
                    <?php 
                        ...
                        wpbc_open_meta_box_section( $submit_form_name . 'general', __('Email is sending to Visitor after Editing of booking.', 'booking')   );
...
//          ) return;
        ...
        $js_script .= " jQuery('select[name=\"modification_template_file\"]').on( 'change', function(){    
                                if ( jQuery('select[name=\"modification_template_file\"] option:selected').val() == 'plain' ) {   
        ...
        $js_script .= "\n";                                                     //New Line
        $js_script .= " if ( jQuery('select[name=\"modification_template_file\"] option:selected').val() == 'plain' ) {   
                            jQuery('.template_colors').hide();                                    
                        } ";    
        ...
        $js_script .= "     if (  jQuery('#modification_to_name').val() == ''  ) {";
        $js_script .= "         jQuery('#modification_to_name').parent().append('<div class=\'updated\' style=\'border-left-color:#ffb900;padding:5px 10px;\'>". esc_js(__('If empty then title defined as WordPress', 'booking' ))."</div>')";
        $js_script .= "     }";
        $js_script .= "     if (  jQuery('#modification_from_name').val() == ''  ) {";
        $js_script .= "         jQuery('#modification_from_name').parent().append('<div class=\'updated\' style=\'border-left-color:#ffb900;padding:5px 10px;\'>". esc_js(__('If empty then title defined as WordPress', 'booking' ))."</div>')";
        $js_script .= "     }";
        $js_script .= "  }); ";
          // Show Warning messages if "From" Email DNS different from current website DNS
        $js_script .= " jQuery(document).ready(function(){ ";
        
        $js_script .= "     var wpbc_email_from = jQuery('#modification_from').val();";    // from@wpbookingcalendar.com 
        ...
        $js_script .= "         jQuery('#modification_from').parent().append('<div class=\'updated\' style=\'border-left-color:#ffb900;padding:5px 10px;\'>". esc_js(__('Email different from website DNS, its can be a reason of not delivery emails. Please use the email withing the same domain as your website!', 'booking' ))."</div>')";
        $js_script .= "     }";
...
add_action('wpbc_menu_created',  array( new WPBC_Settings_Page_Email_Modification() , '__construct') );    // Executed after creation of Menu
 ...
 */
function wpbc__get_replace_shortcodes__email_modification( $booking_id, $bktype, $formdata ) { 
 ...
 */                            
function wpbc_send_email_modified( $booking_id, $bktype, $formdata ) {
    ...
    $previous_active_user = apply_bk_filter( 'wpbc_mu_set_environment_for_owner_of_resource', -1, $bktype );    // MU
        ...
        $replace = wpbc__get_replace_shortcodes__email_modification( $booking_id, $bktype, $formdata );
